...
FILTERED_RANGE_VALUES = (50.0,)
PREVIEW_COUNT = 8


class BlueseaUartRangeTester(Node):
    def __init__(self):
        super().__init__('bluesea_uart_range_tester')
    ...
    def timer_callback(self):
        ...
        for index, distance in enumerate(ranges):
            if not math.isfinite(distance):
                continue
            if any(
                math.isclose(distance, filtered, rel_tol=0.0, abs_tol=0.001)
                for filtered in FILTERED_RANGE_VALUES
            ):
                continue
